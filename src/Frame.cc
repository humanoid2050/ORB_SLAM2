...
Frame::Frame(const cv::UMat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
...
void Frame::ExtractORB(int flag, const cv::UMat &im)
{
    if(flag==0)
...
void Frame::ComputeImageBounds(const cv::UMat &imLeft)
{
    if(mDistCoef.at<float>(0)!=0.0)
    {
